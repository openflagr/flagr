...
func TestSendNotification(t *testing.T) {
	t.Run("sends to multiple notifiers concurrently", func(t *testing.T) {
		...
		sendNotification(OperationCreate, EntityTypeFlag, 1, "test-flag", "description", "", "", "", "user")
		...
		for _, mock := range []*MockNotifier{mock1, mock2, mock3} {
			...
			assert.Equal(t, "test-flag", sent[0].EntityKey)
		}
	})
	...
	t.Run("handles errors from some notifiers", func(t *testing.T) {
		...
		sendNotification(OperationUpdate, EntityTypeFlag, 2, "test-flag-2", "", "", "", "", "")
	...
	t.Run("does nothing when notifiers is empty", func(t *testing.T) {
		...
		// Should not panic
		sendNotification(OperationCreate, EntityTypeFlag, 1, "test", "", "", "", "", "")
	})

	t.Run("SendFlagNotification sends with correct entity type", func(t *testing.T) {
		mock := NewMockNotifier()
		Notifiers = nil
		stubs := gostub.Stub(&Notifiers, []Notifier{mock})
		defer stubs.Reset()

		SendFlagNotification(OperationCreate, 42, "my-flag", "my description", "", "", "", "creator")
		...
		assert.Equal(t, OperationCreate, sent[0].Operation)
		assert.Equal(t, EntityTypeFlag, sent[0].EntityType)
		assert.Equal(t, uint(42), sent[0].EntityID)
		assert.Equal(t, "my-flag", sent[0].EntityKey)
		assert.Equal(t, "my description", sent[0].Description)
...
func TestSendNotificationConcurrency(t *testing.T) {
	t.Run("concurrent sends are handled safely", func(t *testing.T) {
		...
		for i := 0; i < 50; i++ {
			...
			go func(id uint) {
				defer wg.Done()
				SendFlagNotification(OperationCreate, id, "flag", "", "", "", "", "")
			}(uint(i))
		}
...
func TestNotifierDirectSend(t *testing.T) {
	t.Run("can send to notifier directly with context", func(t *testing.T) {
		...
		sent := mock.GetSentNotifications()
		assert.Len(t, sent, 1)
		assert.Equal(t, "direct-test", sent[0].EntityKey)
	})
}
