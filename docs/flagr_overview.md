...
**Eval-only** - `json_file` / `json_http` drivers force eval-only mode: health, evaluation, and eval-cache export only. The UI works as a read-only browser fed by the export endpoint; writes under `/flags` return 403. Details: [behavioral contracts: eval-only](flagr_behavioral_contracts.md#eval-only).
