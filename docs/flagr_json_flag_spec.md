...
Flagr can serve flags from a **JSON file or URL** instead of a database. The evaluation engine is identical; what changes is the authoring workflow. Flags live in a file you control, edits happen through pull requests, and a running Flagr instance becomes a read-only consumer that polls for updates. Because `json_file` and `json_http` both put the server into eval-only mode, the exposure endpoint and database are gone. The UI stays available as a **read-only browser**: it discovers eval-only via `GET /health` (`evalOnlyMode`), shows the current flag state from the export dump, and keeps the Debug Console for live evaluation. Write APIs under `/api/v1/flags` return 403 — the JSON source is the only write path. The behavioral rules for that mode are on [Behavioral contracts](flagr_behavioral_contracts.md#eval-only).
