...
1. **No `$` prefix** — The conditions library (`github.com/zhouzhuojie/conditions`) does not support `$` in variable names. Keys use `@` prefix: `@ts`, `@ts_hour`, `@http_x_environment`.
...
6. **Numeric time values as float64** — All injected values (`@ts`, `@ts_hour`, `@ts_weekday`, `@ts_month`) are `float64` to survive JSON round-tripping (`int64` → `float64` via HTTP API). The conditions library's `>=`/`<=` operators only work with numbers, not strings.

7. **sync.Once cached header matching** — The exact and prefix header match sets are computed once via `sync.Once`, not per-request. `ResetHeaderMatchCache()` is available for tests that change config.

8. **Case-insensitive matching** — Both exact and prefix header matching use lowercase comparison for consistency.
...
| `@ts` | `time.Now().UTC().Unix()` | Current server time as Unix epoch seconds (`float64` for JSON compatibility). Updated per-request. | `1751666400`, `1752537600` | Schedule activation/deprecation; active time windows | `{@ts} GTE 1752537600` |
...
| `pkg/handler/builtin_context.go` | **New** — `InjectBuiltInContext()` function, key constants, header injection with sync.Once cached matching |
| `pkg/handler/builtin_context_test.go` | **New** — 22 unit tests for injection logic |
| `pkg/handler/builtin_context_integration_test.go` | **New** — 10 integration tests for eval with built-in context |
| `pkg/handler/builtin_context_bench_test.go` | **New** — 6 benchmarks for hot path impact |
| `pkg/handler/eval.go` | Modified `PostEvaluation` and `PostEvaluationBatch` to call `InjectBuiltInContext` per entity |
| `browser/flagr-ui/src/components/ConstraintValueCell.vue` | **New** — shared value cell with `@ts*` hint tooltip |
| `browser/flagr-ui/src/components/ConstraintAddRow.vue` | Uses `ConstraintValueCell` |
| `browser/flagr-ui/src/components/ConstraintExistingRow.vue` | Uses `ConstraintValueCell` |
| `browser/flagr-ui/src/helpers/contextKeyHints.ts` | **New** — `@ts*` value hint formatter |
| `browser/flagr-ui/src/styles/app.scss` | Mobile breakpoint (768px) for constraint rows |
| `integration_tests/integration_test.go` | 2 E2E tests with `isLegacyIntegrationBaseline()` skip |

### Key Implementation Details

- `InjectBuiltInContext(entityContext any, r *http.Request) any` — main entry point, returns original value unchanged when disabled (zero cost)
- `injectHTTPHeaders(ctx map[string]any, r *http.Request)` — iterates `r.Header` directly (no `Clone()`), handles `Host` via `r.Host`
- `getHeaderMatchSets() (map[string]bool, map[string]bool)` — sync.Once cached, case-insensitive exact and prefix matching
- `ResetHeaderMatchCache()` — test helper to reset sync.Once when config changes between subtests
- All injected values are `float64` to survive JSON round-tripping
- Empty header values skipped
- Multi-value headers joined with `, `
- Batch eval uses per-entity injection matching single-eval semantics (in-place mutation)
...
- 6 benchmarks: hot path impact measured
- 58 UI tests: all pass (contextKeyHints + constraint components)
- 2 E2E integration tests: positive + negative match, legacy skip
