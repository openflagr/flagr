...
Deploy recipes: [Self-hosting](flagr_self_host.md). Kubernetes: install the in-repo chart (`helm/`) and pass `FLAGR_*` via `env` / `envFrom`. Strategies (Docker, Compose, Helm SQLite/HA/GitOps/SQL): [Deployment strategy](flagr_self_host.md#deployment-strategy).
