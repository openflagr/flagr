...
Prefix whitelist matching (JWT and basic) uses `util.HasSafePrefix`. A path containing `..` never matches a whitelist prefix. Independently, `rejectDotDotPath` rejects any `..` path with **401** before auth or the eval-only flags deny, so `/api/v1/health/../flags` cannot skip health-whitelist and land on flags. [behavioral contracts: eval-only](flagr_behavioral_contracts.md#eval-only).
