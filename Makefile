...
test: verifiers
	@go test -race -covermode=atomic -coverprofile=coverage.txt github.com/rexmont/flagr/pkg/...

ci: test

build:
	@echo "Building Flagr Server to $(PWD)/flagr ..."
	@CGO_ENABLED=1 go build -o $(PWD)/flagr github.com/rexmont/flagr/swagger_gen/cmd/flagr-server
